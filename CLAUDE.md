...
| `docs/DEPLOYMENT-SURFACES.md` | Production deployment surfaces: GitHub Actions, compose, static website publish, manual scripts, and Ansible roles | Working on deployment wiring, deciding whether a script is active vs legacy/manual, or debugging production topology mismatches |
...
| `docs/DOS-HARDENING.md` | DoS hardening across RPC, P2P, storage replay, mempool admission, block validation, and snapshot loading | Working on request/message limits, malformed untrusted data handling, peer abuse scoring, or debugging resource-exhaustion symptoms |
...
| `docs/EXPLORER-PRESENTATION.md` | Explorer presentation helpers: amounts, statuses, timing, badges, links, and HTML escaping | Working on `website/src/explorer-format.ts`, transaction amount/status display, dashboard cards, hash links, or XSS-safe HTML rendering |
...
| `docs/OPERATOR-TOOLS.md` | Operator-facing tooling for snapshot conversion/activation, BTC claim generation/broadcast, and production monitoring wrappers | Working on `src/tools/convert-snapshot.ts`, `src/tools/claim-btc.ts`, `scripts/generate-snapshot.sh`, `scripts/activate-snapshot.sh`, `scripts/q.ts`, or related package scripts such as `convert-snapshot`, `claim:generate`, `snapshot:activate`, and `q` |
...
| `docs/WEBSITE-QA.md` | Browser-side QA workflow for the Vite/Tailwind explorer: Playwright visual screenshots, mocked `/api/v1` data, API-error rendering checks, and XSS coverage | Working on `website/src/*`, `website/e2e/*`, `website/playwright.config.ts`, explorer route rendering, API failure states, or screenshot coverage |
