...
${p('The before and after versions of the <span class="font-mono text-xs text-qubit-400">GET /tx/:txid</span> handler show the reduction clearly. The nested loop is replaced by a two-line lookup:')}
${p('<span class="font-mono text-xs text-qubit-400 block bg-surface rounded p-3 mb-2">const block = node.chain.findTransactionBlock(txid)<br>if (block) {<br>&nbsp;&nbsp;const foundTx = block.transactions.find(t => t.id === txid)!<br>&nbsp;&nbsp;res.json(sanitize(foundTx))<br>&nbsp;&nbsp;return<br>}</span>')}
${p('The <span class="font-mono text-xs text-qubit-400">!</span> non-null assertion is correct by construction: <span class="font-mono text-xs text-qubit-400">findTransactionBlock</span> returns the block that was indexed against this txid, so the transaction is guaranteed to be in it.')}
${h2('Memory Trade-off')}
