...
${p('What surprised us: aggregating by address (summing all UTXOs per address) is the right UX decision but changes the claim boundary. You don\'t claim a specific UTXO from the set — you prove ownership of an address balance in the committed snapshot. This actually simplifies the claim flow since users think about addresses, not UTXOs.')}
${h2('The Claim Mechanism')}
${p('BTC claims work as special transactions. A claim transaction has no inputs in the traditional UTXO sense — instead it has a <span class="font-mono text-xs text-qubit-400">claimData</span> field containing the Bitcoin address, the proof material required for that BTC script type, and the new QBTC address.')}
${p('The node verifies the address using its native proof path: ECDSA key-hash checks for legacy and SegWit-v0 claims, Schnorr key checks for Taproot, and ordered script/signature validation for multisig claims. If the address exists in the snapshot with unclaimed balance, the chain mints the QBTC into the new ML-DSA-65 address. First claim wins — no double-claiming.')}
