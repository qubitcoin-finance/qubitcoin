...
- **Address normalization.** RPC handlers lower-case the address before querying (`src/rpc.ts:209`, `:220`), because index keys are stored as the lower-case hex addresses written by `applyBlock`. Querying with mixed case would miss.
