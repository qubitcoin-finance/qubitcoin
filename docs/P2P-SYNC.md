...
`handleVersion` (`server.ts:464`) is the gate. It rejects, with escalating misbehavior penalties, any peer whose payload fails validation: non-integer/negative `height` (+25), malformed `genesisHash` (+25, must pass `isValidHash`), bad `version` (+25), oversized `userAgent` (+10), out-of-range `listenPort` (+10), or non-hex `cumulativeWork` (+10).
